...
static int rpi5_bootinfo_reserve_null_page(struct ukplat_bootinfo *bi)
{
	 ...
	 * FDT setup initially creates a FREE region beginning at 0x0. The
	 * common memregion allocator returns the allocated physical address as
	 * a pointer; allocating from 0x0 would therefore return NULL, which
	 * callers interpret as an allocation failure even though the region was
	 * consumed.
	 *
	 * Reserve only the first page to remove that ambiguity while retaining
	 * the rest of the low RAM. This descriptor initially overlaps the start
	 * of the FREE descriptor. The common coalescer gives RESERVED regions
	 * priority and trims the FREE descriptor from [0, ...] to [__PAGE_SIZE,
	 * ...].
	 */
...
void rpi5_ukplat_entry(void)
{
	...
	rpi5_pl011_puts("rpi5: entering platform setup\r\n");
	 ...
	 * and marks them as unmapped. Later coalescing would reject those
	 * virtual addresses as invalid memory-region descriptors.
	 */
	...
	if (rc)
		UK_CRASH("rpi5: device memory attributes failed: %d\n", rc);
#endif
